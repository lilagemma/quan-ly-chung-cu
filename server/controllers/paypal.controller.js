...
exports.getCurrentMonthStatus = async (req, res, next) => {
  try {
    ...
    // If no maintenance record exists for current month, create one
    if (!maintenance) {
      // Calculate due date (18th of current month)
      const dueDate = new Date(currentYear, currentMonth - 1, 18);
...
exports.createOrder = async (req, res, next) => {
  try {
    ...
    request.requestBody({
      ...
      purchase_units: [
        {
          reference_id: maintenance._id.toString(),
          amount: {
            currency_code: "USD",
            value: maintenance.total_amount.toString(),
          },
          description: `Maintenance ${maintenance.month}/${maintenance.year}`,
        },
      ],
      application_context: {
    ...
    // =========================
    // SAVE ORDER ID (giống Razorpay)
    // =========================
    maintenance.paypal_order_id = order.result.id;
    await maintenance.save();

    // =========================
    // RESPONSE GIỮ NGUYÊN FORMAT CŨ
    // =========================
    res.status(200).json({
      success: true,
      data: {
...
exports.generateMonthlyMaintenance = async (req, res, next) => {
  try {
    ...
    // Calculate due date (18th of target month)
    const dueDate = new Date(targetYear, targetMonth - 1, 18);
    ...
    res.status(200).json({
      success: true,
      message: `Maintenance records generated: ${created} created, ${skipped} skipped (already exist)`,
      data: {
        month: targetMonth,
        year: targetYear,
        created,
